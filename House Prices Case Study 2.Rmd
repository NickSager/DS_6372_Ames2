...
train[524, ] %>% select(sale_price, gr_liv_area)
train[1299, ] %>% select(sale_price, gr_liv_area)
...
if (file.exists("Models/lm_complexZZ.rds")) {
...
data.frame(id = id_col, SalePrice = exp(stepwise_pred)) %>%
  ...
  write_csv("Predictions/complexlm_predictions1.csv")
...
The linear regression model with transformations does better than the interpretable model. The RMSE is 0.0152 on the log scale which translates to roughly a 16% multipliciative change on the original scale. Interpreting this is difficult due to the complexity of the model, but this approximately corresponds to a \$27,000 error in the predicted sale price. This is a significant improvement over the \$34,000 error from the interpretable model. 
